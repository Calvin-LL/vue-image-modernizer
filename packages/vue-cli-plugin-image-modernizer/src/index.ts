...
export default function (
  ...
  api.chainWebpack((config) => {
    ...
    const builtInImageLoaderEscaped = builtInImageLoader.replace(/\\/g, "\\\\").replace(/!/g, "\\x21");

    const builtInImageLoaderOptionsString = JSON.stringify(
      config.module.rule("images").use("url-loader").get("options")
    );

    const builtInImageLoaderOptionsStringEscaped = builtInImageLoaderOptionsString.replace(/\\/g, "\\\\").replace(/!/g, "\\x21")
